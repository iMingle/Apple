AmCharts.AmSerialChart = AmCharts.Class({
	...
	handleWheelReal: function(a, b) {
		if (!this.wheelBusy) {
			...
			0 > a ? d ? (d = this.startTime + e * c, c = this.endTime + 1 * c, this.zoomToDates(new Date(d), new Date(c))) : (d = this.start + e, c = this.end + 1, this.zoomToIndexes(d, c)) : d ? (d = this.startTime - e * c, c = this.endTime -
				1 * c, this.zoomToDates(new Date(d), new Date(c))) : (d = this.start - e, c = this.end - 1, this.zoomToIndexes(d, c))
		}
	},
	...
	getStartTime: function(a) {
		...
		return AmCharts.resetDateToMin(new Date(a), b.minPeriod, 1,
			b.firstDayOfWeek).getTime()
	},
	...
	updateScrollbars: function() {
		...
		this.updateChartScrollbar(this.chartScrollbar,
			this.rotate)
	},
	zoom: function(a, b) {
		var c = this.categoryAxis;
	...
	timeZoom: function(a, b) {
		...
		var f = this.chartData,
			e = this.categoryAxis;
		if (AmCharts.ifArray(f) && (a != this.startTime ||
			b != this.endTime)) {
			var l = e.minDuration(),
				d = this.firstTime,
				n = this.lastTime;
	...
	indexZoom: function(a, b) {
		...
		if (a != this.start || b != this.end) {
			...
			this.zoomScrollbar();
			0 !==
				a || b != this.chartData.length - 1 ? this.showZB(!0) : this.showZB(!1);
			this.updateColumnsDepth();
	...
	zoomScrollbar: function() {
		var a = this.chartScrollbar,
			b = this.categoryAxis;
		a && this.updateScrollbar &&
			(b.parseDates && !b.equalSpacing ? a.timeZoom(this.startTime, this.endTime) : a.zoom(this.start, this.end), this.updateScrollbar = !0)
	},
	...
	countColumns: function() {
		...
		for (n = 0; n < b; n++) {
			...
			var k = e.stackType;
			if ("100%" == k || "regular" == k)
				for (f = !1, l = 0; l < c; l++) d = this.graphs[l], d.valueAxis == e && "column" == d.type && (!f && d.stackable &&
					(a++, f = !0), (!d.stackable && d.clustered || d.newStack) && a++, d.columnIndex = a - 1, d.clustered || (d.columnIndex = 0));
			if ("none" == k || "3d" == k)
				for (l = 0; l < c; l++) d = this.graphs[l], d.valueAxis == e && "column" == d.type && d.clustered && (d.newStack && (a = 0), d.hidden || (d.columnIndex = a, a++));
			if ("3d" == k)
				for (a = 1, n = 0; n < c; n++) d = this.graphs[n], d.newStack && a++, d.depthCount = a
		}
		return a
	},
	parseData: function() {
	...
	dispatchIndexZoomEvent: function() {
		if (this.prevStartIndex != this.start || this.prevEndIndex !=
			this.end) {
			this.startIndex = this.start;
			this.endIndex = this.end;
			var a = this.chartData;
	...
	getClosestIndex: function(a, b, c, d, e, f) {
		...
		return c == n ? l : c < n ? this.getClosestIndex(a, b,
			c, d, e, l) : this.getClosestIndex(a, b, c, d, l, f)
	},
	...
	zoomToDates: function(a, b) {
		...
		if (this.categoryAxis.equalSpacing) {
			...
			b = AmCharts.resetDateToMin(b, this.categoryAxis.minPeriod,
				1);
	...
	formatPeriodString: function(a, b) {
		if (b) {
			...
			for (var n = 0; n < c.length; n++) {
				for (var k = c[n],
					g = 0, h = 0, m, u, r, s, p, v = 0, q = 0, x, t, y, w, C, B = this.start; B <= this.end; B++) {
					var z = f[B];
				...
				a = AmCharts.formatValue(a, {
					...
					sum: g,
					count: h
				}, d, l, k + "\\.", this.usePrefixes, this.prefixesOfSmallNumbers, this.prefixesOfBigNumbers);
		...
		if (-1 != a.indexOf("[[category]]")) {
			...
			if (this.categoryAxis.parseDates) {
				var f = this.balloonDateFormat,
					l = this.chartCursor;
				l && (f = l.categoryBalloonDateFormat); - 1 != a.indexOf("[[category]]") && (f = AmCharts.formatDate(e, f), -1 != f.indexOf("fff") && (f = AmCharts.formatMilliseconds(f, e)), e = f)
			}
	...
	addChartScrollbar: function(a) {
		...
		this.rotate ? void 0 ===
			a.width && (a.width = a.scrollbarHeight) : void 0 === a.height && (a.height = a.scrollbarHeight);
...
AmCharts.Cuboid = AmCharts.Class({
	...
	draw: function() {
		...
		var q, x, t, y, w, C, B, z, A, D = n;
		s && (n = 0);
		if (0 < e || 0 < f) B = v, v = AmCharts.adjustLuminosity(p, -.2), v = AmCharts.adjustLuminosity(p, -.2), q = AmCharts.polygon(b, [0, e, c + e, c, 0], [0, f, f, 0, 0], v, n, 1, g, 0, m), 0 < h && (A = AmCharts.line(b, [0, e, c + e], [0, f, f], g, h, k, r)), x = AmCharts.polygon(b, [0, 0, c, c, 0], [0, d, d, 0, 0], v, n, 1, g, 0, m), x.translate(e,
		...
		1 > Math.abs(d) && (d = 0);
		1 > Math.abs(c) && (c = 0);
		b = 0 === d ? AmCharts.line(b, [0, c], [0, 0], g, h, k, r) : 0 === c ? AmCharts.line(b, [0, 0], [0, d], g, h, k, r) : 0 < u ? AmCharts.rect(b, c, d, l, n, k, g, h, u, m, r) : AmCharts.polygon(b, [0, 0, c, c, 0], [0, d, d, 0, 0], l, n, k, g, h, m, !1, r);
	...
	updateWidth: function() {
		...
		a.frame++;
		var b = a.totalFrames;
		a.frame <= b && (b = a.easing(0, a.frame, 1, a.rw - 1, b), a.width(b),
			setTimeout(function() {
...
AmCharts.CategoryAxis = AmCharts.Class({
	...
	draw: function() {
		...
		if (AmCharts.ifArray(a)) {
			...
			var x = this.firstDayOfWeek,
				...
				a = AmCharts.resetDateToMin(new Date(a[a.length -
					1].time + 1.05 * g), this.minPeriod, 1, x).getTime(),
				y;
			this.endTime > a && (this.endTime = a);
			q = this.minorGridEnabled;
			var w, a = this.gridAlpha,
				C;
			if (this.parseDates && !this.equalSpacing) {
				...
				var l = Math.ceil(this.timeDifference / r) + 5,
					B = h = AmCharts.resetDateToMin(new Date(this.startTime - r), e,
						u, x).getTime();
				s == e && 1 == u && this.centerLabelOnFullPeriod && (v = r * this.stepWidth);
				...
				var z = c.firstTime,
					A = 0;
				q && 1 < u && (w = this.chooseMinorFrequency(u), C = AmCharts.getPeriodDuration(e, w));
				if (0 < this.gridCountR)
					for (b = d; b <= l; b++) {
						m = z + r * (b + Math.floor((B - z) / r)) - A;
						"DD" == e && (m += 36E5);
						m = AmCharts.resetDateToMin(new Date(m), e, u, x).getTime();
						"MM" == e && (q = (m - h) / r, 1.5 <= (m - h) / r && (m = m - (q - 1) * r + AmCharts.getPeriodDuration("DD", 3), m = AmCharts.resetDateToMin(new Date(m),
							e, 1).getTime(), A += r));
						g = (m - this.startTime) * this.stepWidth;
						q = !1;
						this.nextPeriod[s] && (q = this.checkPeriodChange(this.nextPeriod[s], 1, m, h, s));
						y = !1;
						q && this.markPeriodChange ? (q = this.dateFormatsObject[this.nextPeriod[s]], this.twoLineMode && (q = this.dateFormatsObject[s] + "\n" + q, q = AmCharts.fixBrakes(q)), y = !0) : q = this.dateFormatsObject[s];
						t || (y = !1);
						h = AmCharts.formatDate(new Date(m), q);
						if (b == d && !n || b == l && !k) h = " ";
						this.labelFunction && (h = this.labelFunction(h, new Date(m), this, e, u, p).toString());
						this.boldLabels && (y = !0);
						p = new this.axisItemRenderer(this, g, h, !1, v, 0, !1, y);
						this.pushAxisItem(p);
						p = h = m;
						if (!isNaN(w))
							for (g = 1; g < u; g += w) this.gridAlpha = this.minorGridAlpha, q = m + C * g, q = AmCharts.resetDateToMin(new Date(q), e, w, x).getTime(), q = new this.axisItemRenderer(this, (q - this.startTime) * this.stepWidth), this.pushAxisItem(q);
						this.gridAlpha = a
					}
			} else if (!this.parseDates) {
				if (this.cellWidth = this.getStepWidth(b), b < l && (l = b), f += this.start, this.stepWidth = this.getStepWidth(b), 0 < l)
					for (t = Math.floor(b / l), w = this.chooseMinorFrequency(t), g =
						f, g / 2 == Math.round(g / 2) && g--, 0 > g && (g = 0), l = 0, this.end - g + 1 >= this.autoRotateCount && (this.labelRotation = this.autoRotateAngle), b = g; b <= this.end + 2; b++) {
						p = !1;
						0 <= b && b < this.data.length ? (s = this.data[b], h = s.category, p = s.forceShow) : h = "";
						if (q && !isNaN(w))
							if (b / w == Math.round(b / w) || p) b / t == Math.round(b / t) || p || (this.gridAlpha = this.minorGridAlpha, h = void 0);
							else continue;
						else if (b / t != Math.round(b / t) && !p) continue;
						g = this.getCoordinate(b - f);
						p = 0;
						"start" == this.gridPosition && (g -= this.cellWidth / 2, p = this.cellWidth / 2);
						x = !0;
						tickShift =
							p;
						"start" == this.tickPosition && (tickShift = 0, x = !1, p = 0);
						...
						p = new this.axisItemRenderer(this, g, h, x, B, p, void 0, y, tickShift, !1, s.labelColor);
						p.serialDataItem = s;
						this.pushAxisItem(p);
						this.gridAlpha = a
					}
				...
				h = AmCharts.resetDateToMin(new Date(this.startTime - r), e, u, x).getTime();
				...
				r = g;
				if (this.data[g].time != AmCharts.resetDateToMin(new Date(this.data[g].time), e, u, x).getTime())
					for (x = 0, y = h, b = g; b < v; b++) m = this.data[b].time, this.checkPeriodChange(e, u, m, y) && (x++, 2 <= x && (r = b, b = v), y = m);
				q && 1 < u && (w = this.chooseMinorFrequency(u), AmCharts.getPeriodDuration(e, w));
				if (0 < this.gridCountR)
					for (b = g; b < v; b++)
						if (m = this.data[b].time, this.checkPeriodChange(e,
							u, m, h) && b >= r) {
							g = this.getCoordinate(b - this.start);
							q = !1;
							this.nextPeriod[s] && (q = this.checkPeriodChange(this.nextPeriod[s], 1, m, h, s));
							y = !1;
							q && this.markPeriodChange ? (q = this.dateFormatsObject[this.nextPeriod[s]], y = !0) : q = this.dateFormatsObject[s];
							h = AmCharts.formatDate(new Date(m), q);
							if (b == d && !n || b == l && !k) h = " ";
							C ? C = !1 : (t || (y = !1), g - this.previousPos > this.safeDistance * Math.cos(this.labelRotation * Math.PI / 180) && (this.labelFunction && (h = this.labelFunction(h, new Date(m), this, e, u, p)), this.boldLabels && (y = !0), p = new this.axisItemRenderer(this,
								g, h, void 0, void 0, void 0, void 0, y), x = p.graphics(), this.pushAxisItem(p), p = x.getBBox().width, AmCharts.isModern || (p -= g), this.previousPos = g + p));
							p = h = m
						} else isNaN(w) || (this.checkPeriodChange(e, w, m, B) && (this.gridAlpha = this.minorGridAlpha, g = this.getCoordinate(b - this.start), q = new this.axisItemRenderer(this, g), this.pushAxisItem(q), B = m), this.gridAlpha = a)
			}
			for (b = 0; b < this.data.length; b++)
				if (n = this.data[b]) k = this.parseDates && !this.equalSpacing ? Math.round((n.time - this.startTime) * this.stepWidth + this.cellWidth / 2) :
					this.getCoordinate(b - f), n.x[this.id] = k;
			n = this.guides.length;
			for (b = 0; b < n; b++) k = this.guides[b], t = t = t = a = d = NaN, w = k.above, k.toCategory && (t = c.getCategoryIndexByValue(k.toCategory), isNaN(t) || (d = this.getCoordinate(t - f), k.expand && (d += this.cellWidth / 2), p = new this.axisItemRenderer(this, d, "", !0, NaN, NaN, k), this.pushAxisItem(p, w))), k.category && (t = c.getCategoryIndexByValue(k.category), isNaN(t) || (a = this.getCoordinate(t - f), k.expand && (a -= this.cellWidth / 2), t = (d - a) / 2, p = new this.axisItemRenderer(this, a, k.label, !0, NaN,
	...
	choosePeriod: function(a) {
		var b = AmCharts.getPeriodDuration(this.periods[a].period, this.periods[a].count),
			c =
			Math.ceil(this.timeDifference / b),
			d = this.periods;
