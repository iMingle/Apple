AmCharts.AmXYChart = AmCharts.Class({
	...
	updateData: function() {
		...
		for (k = 0; k < b.length; k++)
			if (g = b[k], g.data = a, g.zoom(0, c), g = g.valueField) {
				var m;
		...
		for (k = 0; k < b.length; k++) g = b[k], g.maxValue = e, g.minValue = f;
			...
			for (d = 0; d < a.length; d++) {
				...
				for (k = 0; k < c.length; k++) {
					...
					for (m = 0; m < b.length; m++) {
						var h = b[m],
							q = h.id;
						if (h.xAxis.id == g || h.yAxis.id == g) {
							var n = {};
							n.serialDataItem = e;
							n.index = d;
							var p = {},
								l = Number(f[h.valueField]);
							isNaN(l) || (p.value = l);
							l = Number(f[h.xField]);
							isNaN(l) || (p.x = l);
							l = Number(f[h.yField]);
							isNaN(l) || (p.y = l);
							l = Number(f[h.errorField]);
							isNaN(l) || (p.error = l);
							n.values = p;
							this.processFields(h, n, f);
							n.serialDataItem =
								e;
							n.graph = h;
							e.axes[g].graphs[q] = n
	...
	addChartScrollbar: function(a) {
		...
		if (a) {
			...
			this.hideXScrollbar || (b = new AmCharts.SimpleChartScrollbar(this.theme), b.skipEvent = !0, b.chart = this, this.listenTo(b,
	...
	handleAxisSelfZoom: function(a) {
		if ("H" == a.valueAxis.orientation) {
			...
			this.horizontalPosition = a;
			this.widthMultiplier = c
		} else c = this.fitMultiplier(a.multiplier), a = -a.position * c, b = -(this.plotAreaHeight * c - this.plotAreaHeight), a < b && (a = b), this.verticalPosition =
			a, this.heightMultiplier = c;
		this.zoomChart();
		c = this.graphs;
