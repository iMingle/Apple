AmCharts.AmPieChart = AmCharts.Class({
		...
		if (AmCharts.ifArray(e)) {
			if (0 < this.realWidth && 0 < this.realHeight) {
				...
				var d = AmCharts.toCoordinate,
					k = d(this.marginLeft, b),
					a = d(this.marginRight, b),
					t = d(this.marginTop, h) + this.getTitleHeight(),
					m = d(this.marginBottom, h),
					w, x, g, v = AmCharts.toNumber(this.labelRadius),
				...
				for (k = 0; k < e.length; k++)
					if (a = e[k], !0 !== a.hidden && 0 < a.percents) {
						var n = 360 * a.percents / 100,
							q = Math.sin((m + n / 2) / 180 * Math.PI),
							y = -Math.cos((m + n / 2) / 180 * Math.PI) * (b / g),
							p = this.outlineColor;
						p || (p = a.color);
						var z = this.alpha;
	...
	checkOverlapping: function(e, f, c, b, h) {
		var d, k, a = this.chartData,
			t = a.length,
			m = f.label;
		if (m) {
			if (!0 === b)
				for (k = e + 1; k < t; k++) a[k].labelQuarter == c && (d = this.checkOverlappingReal(f, a[k], c)) && (k = t);
			else
				for (k = e - 1; 0 <= k; k--) a[k].labelQuarter == c && (d = this.checkOverlappingReal(f, a[k], c)) && (k = 0);
			!0 === d && 100 >
